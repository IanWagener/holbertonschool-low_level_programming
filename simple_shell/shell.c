...
int main(__attribute__((unused)) int argc, __attribute__((unused)) char **argv, char **env)
{
        ...
        while (1) {
                ...
                if (strings_compare(command[0], "exit") == 0) {
                        ...
                        if (pid == -1) {
                                write(2, "Fork failed!", 12); /* if something went wrong */
                        } else if (pid == 0) {
                                if (command[0][0] != '/' ) {
                                        path = get_path(env,command[0]);
                                } else {
                                        path = command[0];
                                } if (execve(path,command,env) == -1) { /* checks if program exists */
...
void print_string(char *str){ /* function to print a string */
        print_char(str[0]);
        while (*str != '\0') {
                str++;
                print_char(*str);
...
int print_char(char c) /* function to print a char */
{
        return (write(1, &c, 1));
}

void print_number(int n) /*function to print a number*/
{
        int v,t,d,i,u;
        char p;
        if (n == 0) {
                print_char('0'); /* Print '0' if n = 0. */
        } else {
                if (n < 0) {
